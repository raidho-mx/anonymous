 ...
 */
	...
	$stripedId = strtolower( IDcleaner($rawId) );
	?>
...
<div class="sidebar_post three columns offset-by-one"><?php
	...
	if($luckyAd) : ?>
		...
		<div class="ad_group">
			<div class="ad_google_uno">
				<a href="<?php echo $url; ?>"<?php if(!empty($stripedId)) echo ' id="'.$stripedId.'" data-title="'.$rawId.'"'; ?> title="<?php echo $luckyAd['squares'][0]['description']; ?>"<?php if(!empty($stripedId)) { ?>class="register_ga" onClick="ga('send', 'event', 'imagen: <?php echo $rawId;?>', 'click', location.pathname);"<?php } ?> target="_blank">
	...
	// Recent
	$recent = new WP_Query( array( 'post_type' => 'post', 'posts_per_page' => 4 ) );
